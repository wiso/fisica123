...
Per una definizione operariva, poiché non esistono cariche magnetiche, per esplorare il campo, si può usare un aghetto magnetico di prova. Si scopre che un filo rettilineo attraversato da una corrente crea un campo magnetico le cui linee di forza si chiudono su se stesso.
...
\begin{legge}[Biot--Savart]
  ...
  \end{equation}
\end{legge}
con $k$ una costante che dipende dalle unità di misura. Nel Sistema Internazionale si pone:
\[k=\frac{\mu_0}{2\pi}\]
con $\mu_0$ la permeabilità magnetica del vuoto:
\[
  \mu_0=\SI[parse-numbers = false]{4\pi 10^{-7}}{\henry\per\meter}
\]
...
\end{equation}
\begin{Es}[spira circolare lungo l'asse\index{spira}]
  ...
  \[\ve B\left(\ve r\right)=\frac{\mu_0}{4\pi}\int_l\frac{I\ud\ve l\times\left(\ve r-\ve r\,'\right)}{\norm{\ve r-\ve r\,'}^3}=\frac{\mu_0}{4\pi}I\int_0^{2\pi}\frac{R\ud\theta\ver e_\theta\times\left(w\ver e_w-R\ver e_r\right)}{\left(R^2+w^2\right)^{\frac{3}{2}}}\]
  Ricordando che:
...
\begin{equation}
  \ve B(d)=\frac{\mu_0}{2\pi}I_1\frac{\ver k\times\ver \jmath}{d}=-\frac{\mu_0}{2\pi}\frac{I_1}{d}\ver \imath
\end{equation}
...
Consideriamo un percorso che non concatena nessuna corrente, per esempio dato dall'unione da due archi di circoferenza (2 e 4) di raggio $r_1$ e $r_2$ che sottendono lo stesso angolo $\theta$, collegati da dei segmenti radiali (1 e 3):
...
\begin{equation}
  \rot\ve B=\mu_0\ve J
  \label{eq:teo_ampere_diff}
\end{equation}

\begin{Es}[Filo con raggio percorso da corrente]
  Sia un filo di raggio $R$ percorso da corrente $I$. Vogliamo calcolare il campo magnetico generato a distanza $r$ dall'asse del filo. Per simmetria il campo dipende solo da $r$ ed è coassiale. Calcoliamo la densità di corrente assumendo che sia uniforme all'interno del filo: dalla definizione $I=\int_S\ve J\cdot\ve n\,\ud a$ ricaviamo che:
  \[J=\frac{I}{\pi R^2}\] all'interno del filo, mentre all'esterno è nulla.

  
  Per il teorema di Ampere \(\oint_C \ve B\cdot\ud\ve l=\mu_0 I\), considerando un percoso $C$ circolare di raggio $r>R$ e $\ve B$ dipendende solo da $r$:
  \[2\pi r B = \mu_0 I\]
  e quindi
  \[B=\frac{\mu_0}{2\pi}I\frac{1}{r}\]
  All'interno del filo, invece, il percorso $C$ è circolare di raggio $r<R$. In questa sezione la corrente sarà $J\pi r^2=I r^2 / R^2$. Usando ancola il teorema di Ampere:
  \[
...
Sappiamo che le linee del campo di $\ve B$ sono linee chiuse o infinite, non esistono cariche magnetiche. Se prendiamo una superficie qualsiasi ci accorgiamo che tante linee di forza entrano, tante ne escono, quindi:
\begin{equation}
...
\begin{gather}
  ...
  \oint_C\ve B\cdot\ud \ve l=\mu_0 I
\end{gather}
...
\end{equation}
dove abbiamo introdotto $\ve r:=(\ve r-\ve r\,')$ che quindi indica il vettore che individua $P$ rispetto a $\ud\ve l$. L'ultima relazione la possiamo anche scrivere come:
\begin{equation}
...
\end{equation}
detto potenziale vettore magnetico. Infatti se calcoliamo la divergenza
\[
\diver \ve B = \diver\rot\ve A = 0
\]
in quanto la divergenza del rotore è sempre nulla.

$\ve A$ non è univocamente determinato, per esempio se scegliamo un certo $\ve A$ potenziale vettore allora anche $\ve A'=\ve A+\grad\psi$ soddisfa la condizione \eqref{vet_magnetico00}:
\[
...
\end{equation}
Questo è sempre possibile, supponiamo che $\diver\ve A=k\neq 0$, allora scelgo $\ve A'=\ve A +\grad\psi$ tale che $\nabla^2\psi = -k$, allora $\diver \ve A'=0$. Sappiamo dalla circuitazione di Ampere \eqref{eq:teo_ampere_diff} che:
\begin{equation}
  \rot\ve B=\mu_0\ve J
\end{equation}
...
\begin{equation}
  ...
  \label{eq:potenziale_vettoriale_dipolo_magnetico}
\end{equation}
Ricordando $\ve A\times(\ve B\times \ve C)=\ve B(\ve A\cdot \ve C)-\ve C(\ve A\cdot \ve B)$, il campo:
\begin{equation}
\ve B = \ve\nabla\times\ve A = \frac{\mu_0}{4\pi}\ve\nabla\times\left(\frac{\ve m\times\ve r}{r^3}\right) = \frac{\mu_0}{4\pi} \left(\frac{ 3\ve r\left(\ve m\cdot \ve r\right)}{r^5}-\frac{\ve m}{r^3}\right)
\label{eq:campo_magnatico_dipolo_magnetico}
\end{equation}

\begin{Es}[Orbita magnetica]
  Assumendo che il campo magnetico della Terra sia dovuto ad un momento magnetico pari a $\mu_M=\SI{8E22}{\ampere\meter^2}$ posizionato al centro e diretto come l'asse di rotazione, valutare quale deve essere la relazione tra il momento di una protone e il raggio di un'orbita circolare equatoriale intorno alla Terra.

  Partendo dall'equazione \eqref{eq:campo_magnatico_dipolo_magnetico} possiamo calcolare il campo magnetico su un piano equatoriale, nel quale $\ve m\cdot \ve r = 0$
  \[
    B = \frac{\mu_0}{4\pi}\frac{\mu_M}{r^3}
  \]
  nella direzione opposta del momento magnetico. Per un'orbita circolare dobbiamo imporre che la forza di Lorentz sia diretta verso il centro e di modulo $m\frac{v^2}{r}$. Il modulo della forza deve essere
  \[
  m\frac{v^2}{r} = q v B = qv \frac{\mu_0}{4\pi}\frac{\mu_M}{r^3}
  \]
  da cui si ricava che il momento è
  \[
  p = \frac{\mu_0}{4\pi}\frac{q\mu_M}{r^2}
  \]
si noti che questa relazione è molto diversa da quella che si trova nel caso gravitazionale $v^2  = G M  / r$
...
Dal teorema di Ampere \eqref{eq:teo_ampere_diff} avevamo che:
\begin{equation}
  \rot\ve B=\mu_0\ve J
\end{equation}
...
Prima di derivare l'equazione delle onde consideriamo dimostriamo che effettivamente possiamo ignorare le cariche libere in un conduttore. Supponiamo che inizialmente ci siano cariche elettriche con densità $\rho(0)$. Studiamo la sua evoluzione temporale. L'equazione di continuità per la carica
\[
...
\]
Quindi la carica si dissipa con un tempo caratterisico $\tau = \frac{\varepsilon}{\sigma}$, che per un conduttore è un tempo brevissimo. Possiamo quindi considerare il conduttore come privo di cariche libere: $\rho=0$.
...
\begin{multline}
  Ak_0e^{jk_0 L}\left\{j\frac{\partial^2 L}{\partial x^2}-\left(k_0\frac{\partial L}{\partial x}\right)^2+j\frac{\partial^2 L}{\partial y^2}-\left(k_0\frac{\partial L}{\partial y}\right)^2+j\frac{\partial^2 L}{\partial z^2}-\left(k_0\frac{\partial L}{\partial z}\right)^2+k_0n^2\right\}\\=0
\end{multline}
semplificando:
\begin{equation}
  \left\{j\left(\frac{\partial^2 L}{\partial x^2}+\frac{\partial^2 L}{\partial y^2}+\frac{\partial^2 L}{\partial z^2}\right)-k_0\left[\left(\frac{\partial L}{\partial x}\right)^2+\left(\frac{\partial L}{\partial y}\right)^2+\left(\frac{\partial L}{\partial z}\right)^2\right]+k_0n^2\right\}=0
\end{equation}
che diventa:
\begin{equation}
  j\nabla^2 L-k_0\norm{\grad L}^2+k_0n^2=0
\end{equation}
...
\begin{equation}
  n=1+\frac{A\lambda^2}{\lambda^2-\lambda_0^2}
  \label{Mayerdisp}
\end{equation}
