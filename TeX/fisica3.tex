...
\begin{Def}[numero di Avogadro\index{numero!di Avogadro}]
	Il numero di Avogadro $N_A$, è il numero di atomi in $\SI{12}{\gram}$ di $^{12}\mathrm{C}$
\end{Def}
...
\begin{Def}[unità di massa atomica]
	Una massa atomica ($\SI{1}{\atomicmassunit}$) è pari al dodicesimo della massa del $^{12}\mathrm{C}$.
\end{Def}


Una mole di $^{12}\mathrm{C}$ corrisponde per definizione a $\SI{12}{\gram}$ di carbonio 12 e in essa sono contenuti $N_A$ atomi, quindi la massa di un atomo di $^{12}\mathrm{C}$ è $\SI{12}{\gram}/N_A$

$$
N_A \times m_{^{12}\mathrm{C}}=\SI{12}{\gram}
$$

$$
m_{^{12}\mathrm{C}} = \SI{12}{\gram} / N_A
$$
quindi il dodicesimo:
\[
	\SI{1}{\atomicmassunit} = \frac{m_{^{12}\mathrm{C}}}{12} = \frac{\si{\gram}}{N_A} = \frac{\si{\gram}}{6.022\ldots\times 10^{23}\;}\simeq \SI{1.6605402E-27}{\kilogram}
\]
...
\begin{equation}
	U=\frac{3}{2}nRT
\end{equation}
Siamo allora indotti a formulare il principio di equipartizione dell'energia, cioè ad assegnare ad ogni grado di libertà un'energia pari a $\frac{1}{2}NkT$ dove $N$ è il numero di molecole e $k$ la costante di Boltzmann.
...
\begin{equation}
	\frac{\ud N}{\ud v_x}=N\left(\frac{m}{2\pi kT}\right)^{\frac{1}{2}}e^{-\frac{mv_x^2}{2kT}}
...
\end{equation}
che è costante con la frequenza e va come $\sqrt{\frac{T}{m}}$.
...
Consideriamo un intervallo di lunghezze d'onda infinitesimo dello spettro di emissione di $n$ corpi opachi in equilibrio termico tra loro e l'ambiente. Poiché i corpi sono opachi non possono trasmettere la radiazione, parte di essa sarà assorbita e parte riflessa. Sia $E$ l'energia che arriva su un corpo. La parte di energia assorbita sarà $aE$, quella riflessa $rE$ dove $a$ è il coefficiente di assorbimento, $r$ di riflessione. Per conservazione dell'energia $E=aE+rE$
\begin{equation}
	(a+r)=1
\end{equation}
Assumento l'equilibrio termico, l'energia emessa da un corpo deve essere uguale all'energia ricevuta. Per il corpo $1$:
\begin{equation}
...
Planck postula che l'energia degli oscillatori sia discreta, multipla intera di $\epsilon$. Il numero di oscillatori aventi l'energia $E_m=m\epsilon$ con $m \in \field{N}$ è:
\begin{equation}
	n_m=n_0 e^{-\frac{E_m}{kT}}=n_0 e^{-\frac{m\epsilon}{kT}}
\end{equation}
...
\end{equation}
essendo una serie geometrica di ragione $0<e^{-\frac{\epsilon}{kT}}<1$. Quindi $n_0=N (1 - e^{-\frac{\epsilon}{kT}})$. Possiamo riscrivere:
\begin{equation}
	n_m=n_0 e^{-\frac{m\epsilon}{kT}} = N (1 - e^{-\frac{\epsilon}{kT}}) e^{-\frac{m\epsilon}{kT}}
\end{equation}
L'energia totale delle particelle con energia $m\epsilon$
\begin{equation}
	E_{m\epsilon}=n_m (m\epsilon)=m\epsilon n_0e^{-\frac{m\epsilon}{kT}}
\end{equation}
L'energia media sarà\footnote{ricordiamo lo sviluppo in serie di $(1-x)^{-2}$:
	\[
...
Possiamo ricavare la legge di Stefan--Boltzmann ricordando che l'emittanza è $c/4 u$

\begin{equation}
W_b = \frac{c}{4} u = \frac{2\pi^5 k^4}{15h^3c^2}T^4
\end{equation}

\begin{figure}
	\centering
	\begin{tikzpicture}
		\begin{axis}[xlabel = \(\lambda\) (\si{\meter}),
				ylabel = {\(R(\lambda)\)(\si{\joule\meter^{-4}})},]
			\addplot[domain=0.:0.00002,samples=400,color=blue] { 4.99 *10^24 / x^5 * 1 / (exp(0.01438/(x * 500)) - 1)};
			\addlegendentry{\(T=\SI{500}{\kelvin}\)}
			\addplot[domain=0.:0.00002,samples=400,color=orange] { 4.99 *10^24 / x^5 * 1 / (exp(0.01438/(x * 700)) - 1)};
			\addlegendentry{\(T=\SI{700}{\kelvin}\)}
			\addplot[domain=0.:0.00002,samples=400,color=red] { 4.99 *10^24 / x^5 * 1 / (exp(0.01438/(x * 1000)) - 1)};
...
\begin{legge}{Doulon--Petit\index{Doulong}\index{Petit}\index{legge!Doulong--Petit}}
	\begin{equation}
...
\begin{align}
	E=&K(\infty)=\frac{1}{2}mv_0^2=\\
	 =&V(\Delta)=\frac{Zze^2}{\giorgi\Delta}
\end{align}
...
\begin{equation}
		\Delta E=E_2-E_1=-Z^2R_\infty hc\left(\frac{1}{n_2^2}-\frac{1}{n_1^2}\right)
\end{equation}
...
\begin{Es}[Riga $\mathrm{H}_\alpha$]
	...
	\begin{multline*}
		\frac{1}{\lambda}=R\left(\frac{1}{n_2^2}-\frac{1}{n_1^2}\right)=R_\infty\frac{\mu}{m_\mathrm{e}}\left(\frac{1}{2^2}-\frac{1}{3^2}\right)=\frac{5}{36}R_\infty\frac{Zm_\mathrm{p}}{Z m_\mathrm{p}+m_\mathrm{e}}\\=\frac{5}{36}R_\infty\frac{1}{1+\frac{m_\mathrm{e}}{m_\mathrm{p}}}\simeq \SI{1.523E6}{\per\meter}
	\end{multline*}
