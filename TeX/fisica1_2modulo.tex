...
$\si{\torr}=\text{mmHg}=\SI[parse-numbers = false]{1/760}{\atmosphere}$. % fix mmHg
...
Si misura in questo modo la pressione atmosferica, in $\text{mmHg}$.\index{pressione! atmosferica} % Fix mmHg
...
\end{figure}
Consideriamo le forze di volume come quelle proporzionali al volume del fluido su cui la forza è applicata. Definiamo
\[\ve f=\frac{\ve F}{m}\]
notare che $\ve f$ è un'accelerazione.

Consideriamo invece le forze di superficie come quelle forze applicate sul fluido che sono proporzionali alla superficie del fluido. Queste forze sono dovute al fluido che lo circonda.
\[F=pA\]

Con queste due forze possiamo richiedere l'equilibrio, per esempio lungo l'asse $x$:

\[f_x\ud m+p(x)\ud y\ud z-p(x+\ud x)\ud y\ud z=0\]
...
\begin{Es}[Forza peso -- Stevino]
   ...
   \]
   In generale introducendo l'energia potenziale associata alla forza $\ve f$, $U_m$:
...
Il lavoro è svolto dalla forze esterne: $\ve F_1, \ve F_2, \ve P$ è $L=\Delta K$. L'equazione di continuità:
