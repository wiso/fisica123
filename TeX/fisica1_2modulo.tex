...
Nel tubo a U (fig.\@\ref{vasicom}) sono introdotti due liquidi non miscibili 1 e 2. Sotto ad A e B c'è solo il fluido 2. A e B sono alla stessa altezza, quindi per la legge di Stevino:
\begin{figure}[htbp]
\centering
...
\label{vasicom}
\end{figure}
\begin{equation*}p_A=p_B\qquad p_0+\rho_1 gh_1=p_0+\rho_2gh_2\end{equation*}
\begin{equation*}\text{legge dei vasi comunicanti: }\frac{\rho_1}{\rho_2}=\frac{h_2}{h_1}\end{equation*}
in particolare se $\rho_1=\rho_2$ allora $h_1=h_2$.
\subsection{Esperimento di \index{Torricelli}\index{esperimento!di Torricelli}Torricelli 1664}
L'esperimento (fig.\@\ref{estor}) consiste nel riempire fino all'orlo un cilindro di mercurio e di rovesciarlo, senza far uscire il liquido, in una bacinella in cui c'è già del mercurio. Il livello del mercurio scende nel cilindro fino al livello di \unita{0.76}\meter.
\begin{figure}[htbp]
\centering
...
\subsubsection{Superfici isobariche\index{superficie!isobarica}}
...
\subsection{Fluido in rotazione\index{moto!circolare!di un fluido}}
\begin{figure}[htbp]
\centering
...
\begin{equation*}f_r\rho=\rho\omega^2 y=\frac{\ud p}{\ud r}\qquad \ud p=\rho\omega^2 y\ud r\end{equation*}
\begin{equation*}\int_{p_\text{asse}}^p\ud p=\int_0^R \rho\omega^2 y\ud r\end{equation*}
\begin{equation*}p-p_\text{asse}=\rho\omega^2\frac{R^2}{2}\qquad p=p_\text{asse}+\frac{\rho\omega^2}{2}R^2\end{equation*}
\begin{equation*}p_\text{asse}=p_0-\rho gz\qquad p=p_0-\rho gz+\frac{\rho\omega^2}{2}R^2\end{equation*}
...
Approccio di Lagrange\index{Lagrange}: si danno le coordinate $x$ di un elemento di flusso in funzione del tempo, è un metodo di analisi complicato.

Approccio di Eulero\index{Eulero}: si studiano le caratteristiche del fluido in un determinato punto. Ci mettiamo in un punto dello spazio e vediamo come variano le grandezze in funzione del tempo. Per descrivere in modo completo bisogna ripetere le misurazioni in modo completo.
...
\end{enumerate}
\end{minipage}
Linee di flusso\index{linee!di flusso}: tangenti alla $\ve v$ della particella. Ha senso se il moto è stazionario, cioè se $\ve v$ è costante nel tempo.

Le linee di flusso danno origine al tubo di flusso\index{tubo!di flusso}. Non è possibile che vi sia fuoriuscita di fluido latente per definizione: $\ve v$ è tangente alle linee di flusso.
...
\begin{equation*}F=\eta A\frac{\ud v}{\ud z}\qquad\ve F=\eta A \ve\nabla v\qquad\eta=\text{coefficiente di viscosità}\end{equation*}
...
\begin{equation*}F_1=(p+\Delta p)2\pi r\ud r\qquad F_2=-p2\pi r\ud r\end{equation*}
\begin{equation*}F_3=L2\pi (r+\ud r)\eta\left. \frac{\ud v}{\ud r}\right|_{r+\ud r}\quad \frac{\ud v}{\ud r}<0\end{equation*}
\begin{equation*}F_4=-L2\pi r\eta\left.\frac{\ud v}{\ud r}\right|_r\end{equation*}
\begin{equation*}0=\sum F=(p+\Delta p)2\pi r\ud r-p 2\pi r\ud r+L 2\pi (r+\ud r)\eta\left.\frac{\ud v}{\ud r}\right|_{r+\ud r}\!\!\!\!\!\!\!\!\!\!-L2\pi r\eta\left.\frac{\ud v}{\ud r}\right|_r\end{equation*}
\begin{equation*}\text{Taylor nell'intorno di $x=r$:\quad} \left.\frac{\ud v}{\ud r}\right|_x=\left.\frac{\ud v}{\ud r}\right|_r+\left.\frac{\ud^2 v}{\ud r^2}\right|_r(x-r)+\ldots\end{equation*}


\begin{equation*}0=\Delta p r\ud r+\eta L\left\{(r+\ud r)\left.\frac{\ud v}{\ud r}\right|_{r+\ud r}\!\!\!\!\!\!\!\!\!\!-r\left.\frac{\ud v}{\ud r}\right|_r\right\}\simeq\end{equation*}
...
Un sistema\index{sistema} è la porzione di materia sotto osservazione, l'ambiente \index{ambiente} tutto il resto. L'universo termodinamico\index{universo!termodinamico} è l'unione del sistema con l'ambiente. Un sistema aperto \index{sistema!aperto} scambia energia e materia con l'ambiente, un sistema chiuso \index{sistema!chiuso} scambia solo energia, un sistema isolato \index{sistema!isolato}non scambia nè energia nè materia. Una parete adiabatica \index{parete!adiabatica}è una parete che non lascia passare calore, una diatermica \index{parete!diatermica}lascia passare calore.
...
\index{termometro}Il termometro è lo strumento per la misurazione della temperatura $T$. Per fare un termometro scelgo una proprietà $X$ che varia in funzione di $T$. Se $X$ varia linearmente in funzione di $T$ allora (in kelvin):
\begin{equation}
...
\begin{equation*}\lambda=\frac{L_{\text{cil}}}{N_{\text{cil}}}=vt\frac{V}{N\pi d^2vt}=\frac{V}{N\pi d^2}\end{equation*}
Essendo $pV=NkT \Rightarrow V/N=kT/p$
\begin{equation*}\lambda=\frac{kT}{\pi d^2 p}\end{equation*}
L'equazione è stata ricavata pensando che le molecole siano bersagli fermi. Le velocità semplificate non sono le stesse. Quella al numeratore è $\overline{v}$ la velocità molecolare media misurata rispetto al recipiente che contiene il gas. Quella al denominatore rappresenta la velocità relativa media $v_\text{rel}$ rispetto alle altre molecole. Si può dimostrare che $v_\text{rel}=\overline{v}\sqrt{2}$ e quindi:
\begin{equation*}\lambda=\frac{kT}{\sqrt{2}\pi d^2p}\end{equation*}
Bisogna tenere anche conto che al di sotto di una certa pressione il libero cammino medio eccede le dimensioni del contenitore e il concetto stesso di libero cammino  medio perde di significato.
...
\begin{equation*}v=\sqrt{\frac{2}{m}E}\end{equation*}
Il numero di molecole che ha una certa velocità è uguale al numero di molecole che ha una certa energia:
\begin{equation*}N(E)\ud E=N(v)\ud v\end{equation*}
\begin{equation*}N(E)=N(v)\frac{\ud v}{\ud E}=N(v)\sqrt\frac{2}{m}\frac{1}{2}E^{-\frac{1}{2}}\end{equation*}
...
\begin{equation}
\overline E=\int_0^\infty E N(E)\ud E=\frac{3}{2}kT
\end{equation}
...
Definiamo la capacità termica specifica\index{capacità!termica!specifica} di una sostanza la quantità di calore necessaria per far aumentare  temperatura di $\Delta T$ di un'unità di massa.
\begin{equation}
...
\index{Doulong}\index{Petit}Doulong e Petit osservarono che il calore specifico molare o capacità termica molare è uguale per tutti i solidi è circa $\unita{25}\joule\per\mole\usk\kelvin$. In realtà questo è il valore limite a cui tendono i solidi per temperature alte. Per temperature tendenti allo zero assoluto la capacità termica molare dei solidi tende a zero.
...
\begin{equation*}Q=Q_1-|Q_2|=-L\end{equation*}
...
\begin{equation*}\eta=1-\frac{T_D-T_A}{T_C-T_B}=1-\left(\frac{V_1}{V_2}\right)^{\gamma-1}\!\!\!\!\!\!\!\!\! =1-\left(\frac{1}{r}\right)^{\gamma-1}\end{equation*}
...
\begin{equation*}\eta_M=\frac{|L_M|}{Q_M}>\eta_C=\frac{|L_C|}{Q_C}=\frac{|L_M|}{Q_C}\end{equation*}
\begin{equation*}\frac{|L_M|}{Q_M}>\frac{|L_M|}{Q_C} \qquad Q_M<Q_C\end{equation*}

Combino la macchina $M$ con la macchina di Carnot fatta funzionare al contrario, trovo un antikelvin. Assurdo, quindi: \begin{equation*}\eta_M\leq\eta_C\end{equation*}
\begin{figure}[htbp]
\centering
...
\begin{equation*}\frac{Q_i}{Q_{i,0}}=-\frac{T_i}{T_0}\qquad Q_{i,0}=-\frac{T_0Q_i}{T_i}\end{equation*}
...
\index{Boltzman}\index{legge!di Boltzman}
Entropia secondo Boltzman\index{entropia!secondo Boltzman}:
\begin{equation}
S=k\log P
\end{equation}
\end{legge}
...
\begin{equation*}S_f=k\log P=kN\log 2=nR\log 2\end{equation*}
...
\end{figure}
Maxwell dice: ``$c=\frac{1}{\sqrt{\varepsilon_0\mu_0}}$'', cioè che la velocità della luce nel vuoto è costante e non dipende dal sistema di riferimento, contrariamente a quanto si ricava dalle trasformate di Galileo. A meno di associare quel valore $c$ ad un sistema di riferimento privilegiato si crea una spaccatura tra l'elettromagnetismo e la meccanica di Newton. Si inventa l'etere, un sistema di riferimento privilegiato che permea tutto l'universo; Michelson--Morley dimostrano che questo non può esistere.\index{etere}
...
\begin{equation*}t_1=\frac{l_1}{c-v}+\frac{l_1}{c+v}=\frac{l_1(2c)}{c^2-v^2}=\frac{2c}{c^2}\frac{l_1}{1-\left(\frac{v}{c}\right)^2}=\frac{2l_1}{c}\frac{1}{1-\left(\frac{v}{c}\right)^2}\end{equation*}
...
\begin{equation*}\dot y'_{O'}=-U\qquad\dot{x'}_{O'}=0\end{equation*}
$O$ vede il proiettile di $O'$ con velocità:
\begin{equation*}\dot y_{O'}=\frac{\dot y'_{O'}\sqrt{1-\beta^2}}{1-\frac{\beta}{c}\dot x'_{O'}}=-\sqrt{1-\frac{u^2}{c^2}}U\end{equation*}
$O$ spara a $O'$ nel verso delle $y$ crescenti e misura la velocità del proiettile:
\begin{equation*}\dot y_{O}=U\qquad \dot x_{O}=0\end{equation*}
e $O'$ vede il proiettile di $O$ con:
\begin{equation*}\dot y'_{O}=\sqrt{1-\frac{u^2}{c^2}}U\end{equation*}
la quantità di moto si deve conservare quindi $O$ vede:
\begin{equation*}m_{O}U+m_{O'}\dot y_{O'}=0\end{equation*}
\begin{equation*}m_{O'}=\frac{m_OU}{\sqrt{1-\frac{u^2}{c^2}}}U=\frac{m_O}{\sqrt{1-\frac{u^2}{c^2}}}\end{equation*}
Considerando $O$ fermo:
\begin{equation*}m_{O}=m_0=\text{massa a riposo}\end{equation*}
\begin{equation*}m=\frac{m_0}{\sqrt{1-\beta^2}}=\gamma m_0\end{equation*}
\begin{equation*}\beta\rightarrow 1\Rightarrow m\rightarrow +\infty\end{equation*}
\section{Quantità di moto\index{quantità di moto!relativistica}}
\begin{equation*}p=mv=\frac{m_0}{\sqrt{1-\beta^2}}\beta c=\gamma m_0 v\end{equation*}
...
\begin{equation*}\beta\rightarrow 1\Rightarrow p\rightarrow +\infty\end{equation*}
...
\begin{equation*}p_x=mv_x-mv_x=0\qquad p_y=mv_y-mv_y=0\end{equation*}
e dopo l'urto:
\begin{equation*}p_x=mv_x-mv_x=0\qquad p_y=mv_y-mv_y=0\end{equation*}
...
\begin{equation*}=\!\int(\ud m\ve v+m\ud\ve v)\ve v=\!\!\int\!(\ud mv^2+mv\ud v)=c^2\!\!\!\int_{v_A}^{v_B}\!\!\!\!\ud m=\left(m\left(v_B\right)-m\left(v_A\right)\right)c^2\end{equation*}
...
\begin{equation*}F=\frac{ma}{1-\beta^2}=\frac{m_0}{1-\beta^2}(1-\beta^2)^{-1/2}a=\underbrace{\frac{m_0}{(1-\beta^2)^{3/2}}}_{\text{massa longitudinale}}a=m_{//}a\end{equation*}\index{massa!longitudinale}
\subsubsection{$\ve F\bot\ve v$}
\begin{equation*}\ve F=m\ve a=\underbrace{\frac{m_0}{\sqrt{1-\beta^2}}}_\text{massa trasversa}\ve a=m_\perp \ve a\end{equation*}\index{massa!trasversa}
...
\begin{equation*}\frac{\ud w}{\ud x}=\frac{\ud(ct)}{\ud x}=c\frac{1}{\frac{\ud x}{\ud t}}=\frac{c}{v}>1\end{equation*}
...
\begin{equation*}w'=ct'=\frac{ct-\beta x}{\sqrt{1-\beta^2}}\end{equation*}
asse $x'$: $w'=0$, cioè
\begin{equation*}w=\beta x\end{equation*}
asse $w'$: $x'=0$, cioè
\begin{equation*}w=\frac{x}{\beta}\end{equation*}
...
\begin{equation*}\left\{\begin{array}{l}
w^2-x^2=1\\
x^2-w^2=1\end{array}\right.\end{equation*}

\begin{equation*}P_1\left\{\begin{array}{l}
x^2-w^2=1\\
w'=0\\
\end{array}\right.\qquad \left\{\begin{array}{l}
x^2(1-\beta^2)=1\\
x=\frac{1}{\sqrt{1-\beta^2}}\end{array}\right.\end{equation*}
\begin{equation*}x'=\dfrac{\dfrac{1}{\sqrt{1-\beta^2}}-\beta w}{\sqrt{1-\beta^2}}=\dfrac{\dfrac{1}{\sqrt{1-\beta^2}}-\beta^2 x}{\sqrt{1-\beta^2}}=\dfrac{\dfrac{1}{\sqrt{1-\beta^2}}-\dfrac{\beta^2}{\sqrt{1-\beta^2}}}{\sqrt{1-\beta^2}}=1\end{equation*}
\begin{equation*}P_1(1,0)\qquad P_2(0,1)\end{equation*}
\begin{equation*}\overline{OP_1}>\overline{01}\qquad\overline{OP_2}>\overline{01}\end{equation*}
...
\begin{equation*}N=\nu t=\nu_0t-\frac{vt}{\lambda}\end{equation*}
\begin{subequations}
\begin{equation}
...
Il suono è un'onda longitudinale che si propaga di solito nell'aria provocando zone con maggiore pressione e zone con minore pressione e quindi zone con maggior densità e zone di rarefazione. Il suono percepito dall'uomo è quello che ha frequenza $\unita{20\div20000}\hertz$. Sotto ci sono gli infrasuoni, sopra gli ultrasuoni. La velocità del suono è\index{velocità!del suono}:
\begin{equation}
v=\sqrt{\frac{B}{\rho}}
\end{equation}
...
\item[soglia\index{soglia!di udibilità} di udibilità] $I_0=\unita{10^{-12}}\watt\per\meter\squared$\\
...
\item[sensazione\index{sensazione sonora} sonora] non è lineare $10\log_{10} I/I_0$ e si misura in decibel $\deci\bell$\\
\item[soglia del \index{soglia!del dolore}dolore] $\unita{120}\deci\bell$\\
\end{description}
...
%\begin{equation*}\overline{F_1P}=\overline{F_2P}\end{equation*}
...
%\begin{equation*}p\rightarrow{\infty}\Rightarrow p\rightarrow f=\frac{R}{2}\end{equation*}
%\section{Rifrazione\index{rifrazione}}
%La rifrazione avviene quando cambia il mezzo:
%\begin{equation*}\frac{\sin i}{\sin r}=n_{12}=\frac{n_2}{n_1}\end{equation*}
%\begin{equation*}n_{12}=\text{indice di rifrazione relativo ai due mezzi}\end{equation*}
%\begin{equation*}n_1=\text{indice di rifrazione assoluto del primo mezzo (rispetto al vuoto)}>1\end{equation*}
%l percorso non risulta il più corto, ma è il più breve (è un minimo):
%\begin{equation*}t_A^B=t_1+t_2=\frac{AP}{v_1}+\frac{PB}{v_2}=\frac{n_1}{c}AP+\frac{n_2}{c}PB=\frac{1}{c}(%n_1AP+n_2PB)\end{equation*}
%
%\subsection{Lenti sottili\index{lenti!sottili}}
%\begin{equation*}\frac{1}{p}+\frac{1}{q}=\frac{1}{f}=(n-1)\left(\frac{1}{R_1}-\frac{1}{R_2}\right)\end{equation*}
