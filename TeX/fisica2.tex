...
\subsection{Unità di misura}
\[[B]=[mlq^{-1}t^{-1}]=\frac{\si{\newton\second}}{\si{\coulomb\meter}}=\frac{\si{\volt\second}}{\si{\meter\squared}}=\frac{\si{\weber}}{\si{\metre\squared}}=T=\SI{1E4}{\gauss}=10^4\,\text{gauss}\]\index{gauss}
Dove $\si{\weber}=\text{weber}=\si{\volt\second}$, $\si{\tesla}=\text{tesla}=\frac{\si{\weber}}{\si{\meter\squared}}$.\index{weber}
...
Poiché nel teorema entra solo lo divergenza di $\ve S$, allora la sua definizione è aritraria a meno di un rotore.
 ...
 \begin{Es}{Energia dissipata in un cavo condutore}
  ...
  \]
  calcolato sulla superficie è
\[
S = EH = E \frac{B}{\mu_0} = \frac{V}{L} \frac{I}{2\pi R}
\]
e punta radialmente verso l'interno. Abbiamo calcolato il vettore di Poynting sulla superficie, perché vogliamo calcolarne il flusso, cioè l'energia dissipata dal campo elettromatico per unità di tempo nel cavo:
\[
...
\end{eqimp}
Si noti che le due equazioni sono separate per i due potenziali. Usando l'operatore dalembertiano\index{dalembertiano}\index{operatore!dalembertiano} $\square$:
\begin{equation}
